...
1. [Основы Swift](#1)
2. [Все – это объект?](#2)
3. [Три типа объектов](#3)
4. [Переменные](#4)
5. [Функции](#5)
6. [Структура Swift файла](#6)
7. [Область видимости и время жизни объектов](#7)
8. [Свойства и методы](#8)
9. [Модули](#9)
10. [Экземпляры](#10)
11. [Ключевое слово self](#11)
12. [Контроль доступа](#12)
13. [Язык программирования Swift и архитектура приложений](#13)
...
Типы объектов – класс, структура и перечисление – имеют важную общую черту: они могут быть проинициализированы. По сути, когда вы объявляете тип объекта, вы определяете только тип. Создание экземпляра типа означает создание самого объекта – экземпляра данного типа. Например, я могу объявить класс Dog и присвоить этому классу метод:
...
Однако использование ключевого self здесь не является обязательным. Вы можете пропустить его, и код все так же будет работать. В данном случае компиллятор автоматически понимает, что вызываются свойства и методы именно данного экземпляра.
<a name="12"></a>
## Контроль доступа

Пространство имен само по себе не является непреодолимым препятствием для доступа к свойствам и методам. Но такой барьер иногда желателен. Чтобы решить эту проблему, Swift использует ключевое слово private.
...
Сам Swift предоставляет множество мощных библиотек и полезных типов объектов. Более того, большая часть вашего кода при программировании под iOS будет сосредоточена на деталях реальных интерфейсных объектов, которые пользователь может видеть и нажимать.
